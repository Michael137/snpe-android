...
public class ModelOverviewFragmentController extends AbstractViewController<ModelOverviewFragment> {
    ...
    public void onClassificationResult(String[] labels, long javaExecuteTime, long preprocTime, long postprocTime) {
        if (isAttached()) {
            ModelOverviewFragment view = getView();
            view.setClassificationResult(labels);
            view.setJavaExecuteStatistics(javaExecuteTime);
            view.setPreprocStatistics(preprocTime);
            view.setPostprocStatistics(postprocTime);
        }
    }
    ...
    public void onClassificationFailed() {
        if (isAttached()) {
            ...
            getView().setPreprocStatistics(-1);
            getView().setPostprocStatistics(-1);
        }
    }
    ...
    public void loadNetwork() {
        if (isAttached()) {
            ...
            view.setPreprocStatistics(-1);
            view.setPostprocStatistics(-1);
